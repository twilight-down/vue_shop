<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 用户列表区 -->
      <!-- boder 是边框 stripe 是隔行变色 -->
      <el-table :data="userlist" border stripe>
        <!-- :data绑定数据源 lable表示指向哪个标题 prop表示指向哪个数据 -->
        <!-- 设置索引列 -->
        ...
        <el-table-column label="状态" >
          <!-- 同时指定了prop和作用域插槽 那么作用域插槽会覆盖prop prop就不再生效了 -->
        ...
        <el-table-column label="操作" width="180px">
          <template slot-scope="scope">
            <!-- {{scope.row}} -->
            <!-- 修改按钮 -->
            <!-- effect字体颜色 content弹出文本内容 placement文本显示位置 enterable弹出之后是否消失 -->
...
export default {
  ...
  methods:{
   async getUserList(){
    ...
    // 请求成功将用户信息传递给userlist 将总页数传递给total
